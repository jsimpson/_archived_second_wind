...
describe ActivityPresenter do
  context 'any activity' do
    ...
    context 'metric units' do
      let(:presenter) { ActivityPresenter.new(activity, false) }
      subject { presenter }
      ...
      describe '#average_speed' do
        it 'should display in kilometers per hour' do
          expect(activity.average_speed).to be_within(0.1).of(2.40)
          expect(presenter.average_speed).to eql('2.40 km/h')
        end
      end
      ...
      describe '#max_speed' do
        it 'should display in kilometers per hour' do
          expect(activity.max_speed).to be_within(0.1).of(4.19)
          expect(presenter.max_speed).to eql('4.19 km/h')
        end
      end
      ...
      describe '#min_speed' do
        it 'should display in kilometers per hour' do
          expect(activity.min_speed).to be_within(0.1).of(0.3)
          expect(presenter.min_speed).to eql('0.31 km/h')
        end
      end

      describe '#average_pace' do
        it 'should display in minutes per kilometer format' do
          expect(presenter.average_pace).to eql('25:00 min/km')
        end
      end

      describe '#max_pace' do
        it 'should display in minutes per kilometer' do
          expect(presenter.max_pace).to eql('14:19 min/km')
        end
      end

      describe '#min_pace' do
        it 'should display in minutes per kilometer' do
          expect(presenter.min_pace).to eql('193:32 min/km')
        end
      end
